    ...
    try {
      ...
      const groupedWithDetails = await Promise.all(
        Object.values(grouped).map(async (group) => {
          ...
          // Fetch student marks for BCA and this semester
          let studentMarks = {};
          try {
            const marksResponse = await getAllStudentMarksByCourseAndSemester("BCA", group.semesterId);
            ...
            console.error(`Failed to fetch marks for BCA, semester ${group.semesterId}: ${err.message}`);
          }
                      ...
                      <tr key={subIndex}>
                        ...
                        <td className="border border-gray-300 p-2">
                          <button
                            ...
                            onClick={() =>
                              navigate(`/ViewResult/${student.id}/${"BCA"}/${group.semesterId}`)
                            }
                          >
                            View & Update Marks
                          </button>
