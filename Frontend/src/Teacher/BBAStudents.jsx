...
const BBAStudents = () => {
  ...
  const fetchStudents = async () => {
    try {
      ...
      // Filter students for BBA
      const bbaStudents = studentsData.filter(student => student.courseId === "BBA");

      // Group students by semesterId
      const grouped = bbaStudents.reduce((acc, student) => {
      ...
      // Fetch semester names and student marks for each group
      const groupedWithDetails = await Promise.all(
        Object.values(grouped).map(async (group) => {
          ...
          // Fetch student marks for BBA and this semester
          let studentMarks = {};
          try {
            const marksResponse = await getAllStudentMarksByCourseAndSemester("BBA", group.semesterId);
            ...
            console.error(`Failed to fetch marks for BBA, semester ${group.semesterId}: ${err.message}`);
          }
      ...
      <div className="w-full max-w-2xl bg-white bg-opacity-90 shadow-2xl rounded-lg p-8">
        ...
        <div className="space-y-6">
          {groupedStudents.length > 0 ? (
                      ...
                      <tr key={subIndex}>
                        ...
                        <td className="border border-gray-300 p-2">
                          <button
                            ...
                            onClick={() =>
                              navigate(`/ViewResult/${student.id}/${"BBA"}/${group.semesterId}`)
                            }
                          >
                            View & Update Marks
                          </button>
            ...
            <p className="text-gray-600 text-base">No students found for BBA</p>
          )}
        </div>
