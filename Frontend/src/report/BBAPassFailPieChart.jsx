...
const BBAPassFailPieChart = () => {
  const [passFailData, setPassFailData] = useState({});
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Hardcode semester mapping with lowercase semId for 6 semesters
      ...
      try {
        ...
        for (const semId of semesters) {
          try {
            ...
            console.log(`Marks API response for BBA, semester ${semId}:`, marksResponse);

            if (!marksResponse?.data) {
              console.log(`No marks data in response for BBA, semester ${semId}`);
              continue;
            }

            const marksData = marksResponse.data;
            console.log(`Marks data for BBA, semester ${semId}:`, marksData);

            if (!Array.isArray(marksData)) {
              console.error(`Marks data for BBA, semester ${semId} is not an array:`, marksData);
              continue;
            }

            if (marksData.length === 0) {
              console.log(`No marks entries found for BBA, semester ${semId}`);
              continue;
            }
            ...
            console.log(`Pass/Fail stats for BBA, semester ${semId}:`, passFailStats[semId]);
          } catch (err) {
            console.error(`Error fetching marks for BBA, semester ${semId}:`, err.message);
            passFailStats[semId] = { pass: 0, fail: 0 };
          }
        }
        ...
        setError(`Failed to load BBA pass/fail data: ${err.message}`);
        setLoading(false);
      }
    };
  ...
  const pieCharts = Object.keys(passFailData).map(semId => {
    ...
    return (
      <div key={semId} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/3 p-4">
  ...
  return (
    ...
    >
      <div className="w-full max-w-5xl bg-white bg-opacity-90 shadow-2xl rounded-lg p-8">
        <h2 className="text-3xl font-bold text-center mb-6 text-gray-700">BBA Pass/Fail Distribution by Semester</h2>
...
export default BBAPassFailPieChart;