...
const MBAToppersGraph = () => {
  const [studentsData, setStudentsData] = useState({});
  const [chartData, setChartData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Hardcode semester mapping with lowercase semId
      ...
      try {
        ...
        for (const semId of semesters) {
          try {
            ...
            console.log(`Students for MBA, semester ${semId}:`, sortedStudents);
          } catch (err) {
        ...
        setError(`Failed to load MBA students data: ${err.message}`);
        setLoading(false);
      }
    };
    ...
    >
      <div className="w-full max-w-5xl bg-white bg-opacity-90 shadow-2xl rounded-lg p-8">
        <h2 className="text-3xl font-bold text-center mb-6 text-gray-700">
          MBA Students by Semester with Percentages
        </h2>
          ...
          <div className="h-96 mb-6">
            <Bar
              ...
              options={{
                ...
                plugins: {
                  legend: { position: "top" },
                  title: { display: true, text: "MBA Students Percentages by Semester" },
                  tooltip: {
                    callbacks: {
        ...
        <div className="mt-6">
          <h3 className="text-xl font-semibold mb-2">MBA Students Details</h3>
...
export default MBAToppersGraph;