      ...
      try {
        ...
        for (const semId of semesters) {
          try {
            ...
            console.log(`Marks API response for BCA, semester ${semId}:`, marksResponse);

            if (!marksResponse?.data) {
              console.log(`No marks data in response for BCA, semester ${semId}`);
              continue;
            }

            const marksData = marksResponse.data;
            console.log(`Marks data for BCA, semester ${semId}:`, marksData);

            if (!Array.isArray(marksData)) {
              console.error(`Marks data for BCA, semester ${semId} is not an array:`, marksData);
              continue;
            }

            if (marksData.length === 0) {
              console.log(`No marks entries found for BCA, semester ${semId}`);
              continue;
            }
            ...
            console.log(`Students for BCA, semester ${semId}:`, sortedStudents);
          } catch (err) {
            console.error(`Error fetching marks for BCA, semester ${semId}:`, err.message);
            students[semId] = [];
          }
        }
        ...
        setError(`Failed to load BCA students data: ${err.message}`);
        setLoading(false);
      }
    };
    ...
    >
      <div className="w-full max-w-5xl bg-white bg-opacity-90 shadow-2xl rounded-lg p-8">
        <h2 className="text-3xl font-bold text-center mb-6 text-gray-700">
          BCA Students by Semester with Percentages
        </h2>
          ...
          <div className="h-96 mb-6">
            <Bar
              ...
              options={{
                ...
                plugins: {
                  legend: { position: "top" },
                  title: { display: true, text: "BCA Students Percentages by Semester" },
                  tooltip: {
                    callbacks: {
        ...
        <div className="mt-6">
          <h3 className="text-xl font-semibold mb-2">BCA Students Details</h3>
...
export default BCAToppersGraph;